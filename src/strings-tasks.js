...
function findLongestWord(sentence) {
  ...
  const firstIndex = wordLengths.indexOf(maxLength);
...
function reverseWords(str) {
  ...
  const reversedWordsArr = words.map((word) =>
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(helloIndex + 'Hello, '.length, exclamationIndex);
}
...
function getCardId(value) {
  // Define the initial deck of playing cards
  const cards = [
    'A♣',
    '2♣',
    '3♣',
